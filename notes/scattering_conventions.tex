...
\begin{document}
	...
	\begin{equation}
		u_\ell(r) = A_\ell\, k r \left[ F_\ell(kr) \cos\delta_\ell + G_\ell(kr) \sin\delta_\ell \right]\,.
	...
	\begin{appendices}
		...
		\begin{equation}
			...
			\right)\,,
		\end{equation}
		where $\tilde{A}_\ell \equiv A_\ell\, \cos \delta_\ell$.
		...
		\begin{equation}
			...
			-a_\ell\,k^{2\ell+1}\,.
		\end{equation}
		It is possible to re-define the variational asymptotic functions to 
		\begin{equation}
			\begin{cases}
				F_\ell(kr) &\rightarrow \quad
				\tilde{F}_\ell(kr) = 
				\sqrt{k^{2\ell+1}}\,F_\ell(kr)\,,\\
				G_\ell(kr) &\rightarrow \quad
				\tilde{G}_\ell(kr) = -f(r)
				\dfrac{1}{\sqrt{k^{2\ell+1}}}\,G_\ell(kr)\,,
			\end{cases}
		\end{equation}
		where $f(r)$ goes to zero faster than the denominator of $G_\ell(kr)$ for small $r$, but otherwise is basically one outside the core.\\
		Using this redefinition, outside the core the reduced radial function becomes
		\begin{equation}
			u_\ell(kr) = 
			\mathcal{A}_\ell\,kr \left(
			F_\ell(kr)
			+\tilde{R}_{\ell\ell}\,G_\ell(kr)
			\right)
		\end{equation}
		with 
		\begin{equation}
		...
		\end{equation}
		and, for $E\rightarrow 0$, 
		\begin{equation}
			\tilde{R}_{\ell\ell}\rightarrow a_\ell\,.
		\end{equation}
		The problem with this approach is that for $E\rightarrow 0$ the asymptotic function tend to zero or to infinity.
