PROGRAM FITTER_3PX_CHANNELS
  ...
  CHARACTER(LEN=256) :: FILENAME_APPEND
  ...
  ! R_BB matrices
  DOUBLE PRECISION RBB0(RDIM,RDIM), RBB(RDIM,RDIM), K2, HTM, R0, A0
  DOUBLE PRECISION, PARAMETER :: E = 1.D-5 ! Default energy value

  INTEGER :: NEQ, ICH, IC110, IC111
  ...
  IF (COMMAND_ARGUMENT_COUNT() == 3) THEN
    ...
    READ(MESSAGE, *) IC111
    CALL GET_COMMAND_ARGUMENT(3, FILENAME_APPEND)
  ELSE
    STOP "Insert min and max value for the index of C11, max is 184. Insert also filename_eppend"
  END IF
  ENERGIES(1) = 0
  ENERGIES(2) = E
  ...
  INTEGER :: I11, I6, I7, IR, IO_OPEN



  OPEN(UNIT=200, FILE="res_3P0"//TRIM(FILENAME_APPEND)//".dat", ACTION="WRITE", POSITION="APPEND", STATUS="UNKNOWN", IOSTAT=IO_OPEN)
  IF (IO_OPEN /= 0) THEN
    WRITE(*,*) "Error opening file res_3P0", FILENAME_APPEND, " for writing."
    STOP
  END IF
  OPEN(UNIT=201, FILE="res_3P1"//TRIM(FILENAME_APPEND)//".dat", ACTION="WRITE", POSITION="APPEND", STATUS="UNKNOWN", IOSTAT=IO_OPEN)
  IF (IO_OPEN /= 0) THEN
    WRITE(*,*) "Error opening file res_3P1", FILENAME_APPEND, " for writing."
    STOP
  END IF
  OPEN(UNIT=202, FILE="res_3P2"//TRIM(FILENAME_APPEND)//".dat", ACTION="WRITE", POSITION="APPEND", STATUS="UNKNOWN", IOSTAT=IO_OPEN)
  IF (IO_OPEN /= 0) THEN
    WRITE(*,*) "Error opening file res_3P2", FILENAME_APPEND, " for writing."
    STOP
  END IF
  ...
  DO IR = 1, NR
    ...
    DO I7  = 1, N7
      ...
      DO ICH = 1, NCHANNELS
        ...
        IF (ABS_DIFF_PROCENT(A0_18(ICH), A0) < 1.D0) &
