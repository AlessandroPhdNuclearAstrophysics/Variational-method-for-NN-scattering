...
CONTAINS
  ...
  SUBROUTINE LOG_MESSAGE_CLASS(LOG_OBJ, MESSAGE, LEVEL, COLOR)
    IMPLICIT NONE
    CLASS(LOGGER), INTENT(INOUT) :: LOG_OBJ
    CHARACTER(LEN=*), INTENT(IN) :: MESSAGE
    INTEGER, INTENT(IN) :: LEVEL
    ...
    IF (PRESENT(COLOR) .AND. LOG_OBJ%COLORED_OUTPUT) THEN
      FULL_MESSAGE = COLOR//TRIM(LOG_OBJ%LOGGER_NAME) // ": " // MESSAGE//COLOR_RESET
    ELSE
      FULL_MESSAGE = TRIM(LOG_OBJ%LOGGER_NAME) // ": " // MESSAGE
    END IF
    ...
    ELSE
      IF (LOG_OBJ%COLORED_OUTPUT .AND. PRESENT(COLOR)) THEN
        WRITE(*, '(A)') COLOR//FULL_MESSAGE//COLOR_RESET
      ELSE
        WRITE(*, '(A)') FULL_MESSAGE
      END IF
    END IF
  ...
  SUBROUTINE LOG_ERR(LOG_OBJ, MESSAGE)
    IMPLICIT NONE
    CLASS(LOGGER), INTENT(INOUT) :: LOG_OBJ
    CHARACTER(LEN=*), INTENT(IN) :: MESSAGE
    CHARACTER(LEN=1024) :: FULL_MESSAGE
    ...
    IF (LOG_OBJ%MAX_LOG_LEVEL >= 0) THEN
      CALL LOG_MESSAGE_CLASS(LOG_OBJ, MESSAGE, 0, COLOR_RED)
    END IF
  ...
  SUBROUTINE LOG_WARNING(LOG_OBJ, MESSAGE)
    IMPLICIT NONE
    CLASS(LOGGER), INTENT(INOUT) :: LOG_OBJ
    CHARACTER(LEN=*), INTENT(IN) :: MESSAGE

    IF (LOG_OBJ%MAX_LOG_LEVEL >= 1) THEN
      CALL LOG_MESSAGE_CLASS(LOG_OBJ, MESSAGE, 1, COLOR_YELLOW)
    END IF
  ...
  SUBROUTINE LOG_INFO(LOG_OBJ, MESSAGE)
    IMPLICIT NONE
    CLASS(LOGGER), INTENT(INOUT) :: LOG_OBJ
    CHARACTER(LEN=*), INTENT(IN) :: MESSAGE

    IF (LOG_OBJ%MAX_LOG_LEVEL >= 2) THEN
      CALL LOG_MESSAGE_CLASS(LOG_OBJ, MESSAGE, 2, COLOR_CYAN)
    END IF
  ...
  SUBROUTINE LOG_DEBUG(LOG_OBJ, MESSAGE)
    IMPLICIT NONE
    CLASS(LOGGER), INTENT(INOUT) :: LOG_OBJ
    CHARACTER(LEN=*), INTENT(IN) :: MESSAGE

    IF (LOG_OBJ%MAX_LOG_LEVEL >= 3) THEN
      CALL LOG_MESSAGE_CLASS(LOG_OBJ, MESSAGE, 3)
    END IF
  ...
  SUBROUTINE LOG_MESSAGE_COLOR_IMPL(LOG_UNIT, MESSAGE, COLOR)
    ...
    IF (COLOR /= "") THEN
      WRITE(LOG_UNIT, '(A)', ADVANCE='NO') COLOR//MESSAGE//COLOR_RESET
    ELSE
      WRITE(LOG_UNIT, '(A)', ADVANCE='NO') MESSAGE
    END IF
