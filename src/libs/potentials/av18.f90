...
! ---------------------------------
  IF (LPOT.LT.100) THEN
  CALL CONSTS90(LPOT,HC,MPI0,MPIC,MP,MN,ALPHA,MUP,MUN)
  MPI=(MPI0+2.*MPIC)/3.
  MU0=MPI0/HC
  MUC=MPIC/HC
  MU=MPI/HC
...
SUBROUTINE CONSTS90(LPOT, HC, MPI0, MPIC, MP, MN, ALPHA, MUP, MUN)
  IMPLICIT NONE
  INTEGER, INTENT(IN) :: LPOT
  DOUBLE PRECISION, INTENT(OUT) :: HC, MPI0, MPIC, MP, MN, ALPHA, MUP, MUN
