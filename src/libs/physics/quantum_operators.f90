...
CONTAINS
  ...
  FUNCTION S12_OPERATOR(L, S, J) RESULT(S12_RES)
    ...
    ELSEIF (ABS(J-L)==1) THEN
      IF (J==0) THEN
        ...
        ELSE
          WRITE(*,*) "Error in S12_OPERATOR: Invalid LSJ combination"
          STOP
        ENDIF
      ELSE
  ...
  FUNCTION LS_OPERATOR(LMIN, S, J) RESULT(LS_OP)
    ...
    ELSE
      WRITE(*,*) "Error in LS_OPERATOR: Invalid LSJ combination"
      STOP
    ENDIF
  ...
  FUNCTION L2_OPERATOR(L, COUPLED) RESULT(L2_MAT)
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: L
  ...
  FUNCTION T12_OPERATOR(T, TZ) RESULT(CD)
    IMPLICIT NONE
    ...
    IF (ABS(TZ)==1) THEN
      ...
      CD = -4
    ELSE
      WRITE(*,*) "Error in T12_OPERATOR: Invalid T,TZ combination"
    ENDIF
  END FUNCTION T12_OPERATOR
