...
CONTAINS
  FUNCTION CALCULATE_PHASE_SHIFTS_BLATT_RAD(R_MATRIX, DIM) RESULT(PS)
    ...
    FUNCTION EVALUATE_ZERO_ENERGIES_OBSERVABLES_STAPP(R_MATRIX, DIM, LMIN) RESULT(OBS)
      ...
      ELSEIF (DIM == 2) THEN
        OBS%a1 = OBS_BB%a1
        OBS%e  =-OBS_BB%a1*DABS(OBS_BB%e)*DABS(OBS_BB%a2)/OBS_BB%a2
        OBS%a2 = OBS_BB%a2 + OBS_BB%a1 * OBS_BB%e**2
      ELSE
        PRINT *, "Error: Invalid dimension in EVALUATE_ZERO_ENERGIES_OBSERVABLE_S_STAPP"
      ENDIF
      OBS%STAPP = .TRUE.
