...
! S-MATRIX
  ...
  IF (FIRST_CALL) THEN
    ...
    NNN = VAR_P%NNL * NCH
    ALLOCATE(C(NNN, NNN), CC(NNN, NNN), CCC(NNN, NNN))
    ALLOCATE(CAR(NNN,NCH), CAI(NNN,NCH))
    ALLOCATE(CARR(NNN), CAII(NNN))
    ALLOCATE(XRCOEFF(NCH,NNN), XICOEFF(NCH,NNN))
    ALLOCATE(IPIV(NNN))
    ALLOCATE(BD1(NCH,NCH), BD2(NCH,NCH), BD3(NCH,NCH), BD4(NCH,NCH))
  ELSE
    VAR_P%E = E
    K = DSQRT(2*E*MR) / HC
  ENDIF
...
! Evaluating the matrix elements
  ...
  IF (PRINT_I) CALL PRINT_DIVIDER
  CALL CORE_CORE_MATRIX_ELEMENTS            (C, FIRST_CALL)
...
! Calculating the phase shifts and mixing angles in the Stapp convention
  ...
  CONTAINS
    ...
    SUBROUTINE MATRIX_MULTIPLY(M1,M2,M3)
      IMPLICIT NONE
      DOUBLE PRECISION, INTENT(OUT), DIMENSION(:,:) :: M1
      DOUBLE PRECISION, INTENT(IN), DIMENSION(:,:) :: M2
      DOUBLE PRECISION, INTENT(IN), DIMENSION(:,:) :: M3
      INTEGER :: II, JJ, LL
    ...
    SUBROUTINE R_SECOND_ORDER()
      ...
      IF (FIRST_CALL) THEN
        ALLOCATE(XRCOEFV(NNN, NCH_MAX), XICOEFV(NNN, NCH))
        ALLOCATE(BD5(NCH, NNN), BD6(NCH, NNN))
        ALLOCATE( CD0(NCH, NCH), CD1(NCH, NCH), CD2(NCH, NCH), CD3(NCH, NCH), CD4(NCH, NCH), &
                  CD5(NCH, NCH), CD6(NCH, NCH), CD7(NCH, NCH), CD8(NCH, NCH))
        ALLOCATE(ASS(NCH, NCH), RMAT2_ASYM(NCH, NCH))
        ALLOCATE(RD0(NCH, NCH), RD2(NCH, NCH), RD3(NCH, NCH))
        ALLOCATE(RCI(NCH, NNN), RCIV(NNN, NCH))
        FIRST_CALL = .FALSE.
      ENDIF
  ...
  SUBROUTINE CORE_CORE_MATRIX_ELEMENTS(AM, RESET)
    USE LAGUERRE_POLYNOMIAL_MOD
    USE INTEGRATION_MOD

    IMPLICIT NONE
    DOUBLE PRECISION, INTENT(OUT) :: AM(:,:)
    ...
    DOUBLE PRECISION, ALLOCATABLE :: AKEM(:,:), APEM(:,:)
    DOUBLE PRECISION, ALLOCATABLE :: FUN(:)

    DOUBLE PRECISION, ALLOCATABLE, SAVE :: HM(:,:), AXX(:,:)
    ...
    IF (RESET) THEN
      ...
      ALLOCATE(AKEM(NNN,NNN), APEM(NNN,NNN))
      ALLOCATE(HM(NNN,NNN), AXX(NNN,NNN))
  ...
  SUBROUTINE ASYMPTOTIC_CORE_MATRIX_ELEMENTS(AM, AM1, RESET)
    USE LAGUERRE_POLYNOMIAL_MOD
    USE INTEGRATION_MOD
    IMPLICIT NONE
  ...
  ! Initialize grid with r values
    ...
    IF (RESET) THEN
      ALLOCATE(AXXM1(NNN, NCH), AKEM1(NNN, NCH), APEM(NNN,NCH), APEM1(NNN, NCH))
      ALLOCATE(FBES(NCH, NX), GBES(NCH, NX))
  ...
  SUBROUTINE ASYMPTOTIC_ASYMPTOTIC_MATRIX_ELEMENTS(AM, AM1, AM2, AM3, RESET)
    USE gsl_bessel
    USE INTEGRATION_MOD
    IMPLICIT NONE
    DOUBLE PRECISION, PARAMETER :: K_SMALL = 1.D-8 ! fm^-1
    ...
    AF = VAR_P%AF
    IF (RESET) THEN
      CALL EXPONENTIALLY_GROWING_GRID(H, AF, RANGE, XX, AJ, NXMAX, NX)
      ALLOCATE(FUN(NX+1), FUN1(NX+1), FUN2(NX+1), FUN3(NX+1))
      ALLOCATE(FBES(NCH, NX), GBES(NCH, NX), GBES0(NCH, NX), &
               GBES1(NCH, NX), GBES2(NCH, NX), HNOR(NCH, NX))
    ENDIF
