...
! Calculating the phase shifts and mixing angles in the Stapp convention
  ...
  SUBROUTINE CORE_CORE_MATRIX_ELEMENTS(AM, RESET)
    ...
    ! SI CALCOLA ENERGIA CINETICA
          AKEM(IB,IK) = ZERO
          IF(IAB.EQ.IAK)THEN
            FUN = V0(IL,:)*( V2(IR,:) + 2.D0*V1(IR,:)/XX_CC &
                  -LIK*V0(IR,:)/XX_CC**2 )
            SUM = ZERO
            DO I=1,NX
    ...
    DO IAB = 1, NEQ
      DO IAK = 1, NEQ
        ...
        ! Evaluate the normalization core-irregular (axx1)
          ...
          IF(IAB.EQ.IAK)THEN
            FUN1(1)  = ZERO
            FUN1(2:) = AJ_AC*V0(IL,:)*GBES_AC(IE,LL,:)
        ...
        ! Evaluate the kinetic energy core-irregular (ake1)
          ...
          IF(IAB.EQ.IAK)THEN
            FUN1(1) = ZERO
            FUN1(2:) = AJ_AC*GBES_AC(IE,LL,:)*( V2(IL,:) + 2.D0*V1(IL,:)/XX_AC - LIK*V0(IL,:)/XX_AC**2)
        ...
        ! Evaluate the potential energy core-regular (ape), core-irregular (ape1)
          FUN (1) = ZERO
          FUN1(1) = ZERO
          FUN (2:) = AJ_AC*V0(IL,:)*FBES_AC(IE,LL,:)*VV(:,IAB,IAK)
          FUN1(2:) = AJ_AC*V0(IL,:)*GBES_AC(IE,LL,:)*VV(:,IAB,IAK)
