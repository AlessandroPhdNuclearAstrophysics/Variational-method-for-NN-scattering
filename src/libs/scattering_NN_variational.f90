MODULE SCATTERING_NN_VARIATIONAL
  ...
  PUBLIC :: SET_VARIATIONAL_PARAMETERS, SET_NNL, SET_AF, PREPARE_FG
...
! S-MATRIX
  ...
  IF (FIRST_CALL) THEN
    ...
    IF (.NOT.GRID_READY) CALL PREPARE_GRID
    NNN = VAR_P%NNL * NCH
  ...
  IF (.NOT.GRID_READY .OR. .NOT.BESSELS_READY) THEN
...
! Calculating the phase shifts and mixing angles in the Stapp convention
  ...
  SUBROUTINE CORE_CORE_MATRIX_ELEMENTS(AM, RESET)
    ...
    ! SI CALCOLA ENERGIA CINETICA
          AKEM(IB,IK) = ZERO
          IF(IAB.EQ.IAK)THEN
            FUN = V0_CC(IL,:)*( V2_CC(IR,:) + 2.D0*V1_CC(IR,:)/XX_CC &
                  -LIK*V0_CC(IR,:)/XX_CC**2 )
            SUM = ZERO
            DO I=1,NX
              SUM = SUM + XX_CC(I)**2*FUN(I)*A_CC(I)
            ENDDO
    ...
    ! SI CALCOLA ENERGIA POTENZIALE
          SUM = ZERO
          FUN = V0_CC(IL,:)*V0_CC(IR,:)*V(:,IAB,IAK)
          DO I=1,NX
            SUM = SUM + XX_CC(I)*XX_CC(I)*FUN(I)*A_CC(I)
          ENDDO
  ...
  SUBROUTINE ASYMPTOTIC_CORE_MATRIX_ELEMENTS(AM, AM1, RESET)
    ...
    DOUBLE PRECISION :: APF, ANL, XG, FEXP, RR
    INTEGER :: L, S, J, IE
    DOUBLE PRECISION :: VPW(2,2)
    ...
    DO IAB = 1, NEQ
      DO IAK = 1, NEQ
        ...
        ! Evaluate the potential energy core-regular (ape), core-irregular (ape1)
          FUN (1) = ZERO
          FUN1(1) = ZERO
          FUN (2:) = AJ_AC*V0_AC(IL,:)*FBES_AC(IE,LL,:)*VV(:,IAB,IAK)
          FUN1(2:) = AJ_AC*V0_AC(IL,:)*GBES_AC(IE,LL,:)*VV(:,IAB,IAK)
  ...
  !      IRREGOLARE-REGOLARE (APE1), REGOLARE-REGOLARE (APE2), IRREGOLARE-IRREGOLARE (APE3)
      ...
      FUN2(1) = ZERO
      FUN3(1) = ZERO
      FUN (2:) = AJ_AA*FBES_AA(IE,LL,:)*GBES_AA(IE,LR,1:NX)*VV(:,IAB,IAK)
      FUN1(2:) = AJ_AA*GBES_AA(IE,LL,:)*FBES_AA(IE,LR,1:NX)*VV(:,IAB,IAK)
      FUN2(2:) = AJ_AA*FBES_AA(IE,LL,:)*FBES_AA(IE,LR,1:NX)*VV(:,IAB,IAK)
      FUN3(2:) = AJ_AA*GBES_AA(IE,LL,:)*GBES_AA(IE,LR,1:NX)*VV(:,IAB,IAK)
  ...
  SUBROUTINE PREPARE_FG(ENERGIES, LMAX, TZ)
    USE gsl_bessel
    USE INTEGRATION_MOD
    IMPLICIT NONE
    INTEGER, INTENT(IN) :: LMAX, TZ
  ...
  SUBROUTINE PREPARE_POTENTIAL(CHANNELS)
    ...
    INTEGER :: NC, NEQ_C, I, IEQ, L, S, J, T, TZ, T1Z, T2Z, IL, NL
    LOGICAL :: COUPLED

    NC = SIZE(CHANNELS)
    DO I = 1, NC
      NEQ_C = GET_CHANNEL_NCH(CHANNELS(I))
      J   = GET_CHANNEL_J(CHANNELS(I))
      TZ  = GET_CHANNEL_TZ(CHANNELS(I))
      COUPLED = IS_CHANNEL_COUPLED(CHANNELS(I))
      DO IEQ = 1, NEQ_C
        L = GET_CHANNEL_L(CHANNELS(I), IEQ)
        S = GET_CHANNEL_S(CHANNELS(I), IEQ)
        T = GET_CHANNEL_T(CHANNELS(I), IEQ)
      ENDDO
    ENDDO
  ...
  SUBROUTINE PREPARE_LAGUERRE()
    ...
    DO I = 1, NX
      XG = YY_CC(I)
      DO J = 0, NMX
        ANJ = DSQRT(DGAMMA(J+ONE)*GAMMA**3/DGAMMA(J+3.D0))
