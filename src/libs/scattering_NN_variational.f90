...
! Calculating the S-matrix
  ...
  SUBROUTINE FIT_CHANNELS_LOW_ENERGY(CHANNELS_TO_FIT, KPOW2, PHASE_SHIFTS, FIT_CONSTANTS, ORDER_OF_THE_FIT)
    USE FIT_MODULE
    IMPLICIT NONE
    TYPE(SCATTERING_CHANNEL), INTENT(IN) :: CHANNELS_TO_FIT(:)
    DOUBLE PRECISION, INTENT(IN) :: KPOW2(:)
    ...
    INTEGER :: NEQ, L
    INTEGER :: NCH_TO_FIT, ICH, IEQ, NK2, IE, I, IMIN
    ...
    DO ICH = 1, NCH_TO_FIT
      ...
      CALL FIT_CHANNEL_LOW_ENERGY(CHANNEL_TO_FIT, KPOW2, PHASE_SHIFTS(ICH,:), FIT_CONSTANTS(ICH,:), ORDER_OF_THE_FIT)
    ENDDO
  END SUBROUTINE FIT_CHANNELS_LOW_ENERGY

  SUBROUTINE FIT_CHANNEL_LOW_ENERGY(CHANNEL_TO_FIT, KPOW2, PHASE_SHIFTS, FIT_CONSTANTS, ORDER_OF_THE_FIT)
    USE FIT_MODULE
    IMPLICIT NONE
    ...
    INTEGER :: NEQ, L, ICH, IEQ, NK2, IE, I, IMIN
    DOUBLE PRECISION, ALLOCATABLE :: X(:), Y(:)
    DOUBLE PRECISION :: DELTA
    ...
    DO IEQ = 1, NEQ
      ...
      DO I = 1, NK2
        ...
        IF (ABS(DELTA) > 1.D-6) THEN
          IF (IMIN == -1) IMIN = I
          Y(I) = KPOW2(I)**((2.D0*L + 1.D0)/2.D0) / DTAN(DELTA*PI/180.D0)
        ELSE
          Y(I) = 0.D0
          IMIN =-1
        ENDIF
      ENDDO
      CALL POLYNOMIAL_REGRESSION(Y(IMIN:), X(IMIN:), ORDER_OF_THE_FIT, NK2-IMIN, FIT_CONSTANTS)
    ENDDO
