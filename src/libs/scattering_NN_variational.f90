  ...
  SUBROUTINE PREPARE_CORE_CORE_MATRIX_ELEMENTS()
    ...
    IF (USE_DYNAMIC) THEN
      ...
      IF (.NOT.ALLOCATED(FMAT_CC)) THEN
        ALLOCATE(FMAT_CC(0:7, NCHANNELS, NNN_MAX, NNN_MAX))
      ENDIF
    ENDIF
  ...
  SUBROUTINE PREPARE_ASYMPTOTIC_CORE_MATRIX_ELEMENTS()
    ...
    IF (USE_DYNAMIC) THEN
      ...
      IF (.NOT.ALLOCATED(FMAT_AC_R)) THEN
        ALLOCATE(FMAT_AC_R(0:7, NCHANNELS, NNN_MAX, NCH_MAX))
        ALLOCATE(FMAT_AC_I(0:7, NCHANNELS, NNN_MAX, NCH_MAX))
      ENDIF
    ENDIF
            ...
            ! Evaluate the potential energy core-regular (ape), core-irregular (ape1)
              ...
              ELSE
                DO IPOT = 0, 7
                  ...
                  FMAT_AC_R(IPOT,ICH,IB,IAK) = B5_SINGLE(NX,H5,FUN,1)
                  FMAT_AC_I(IPOT,ICH,IB,IAK) = B5_SINGLE(NX,H5,FUN1,1)
                ENDDO
              ENDIF
  ...
  SUBROUTINE PREPARE_ASYMPTOTIC_ASYMPTOTIC_MATRIX_ELEMENTS
    ...
    IF (USE_DYNAMIC) THEN
      ...
      IF (.NOT.ALLOCATED(FMAT_AA_RR)) THEN
        ALLOCATE(FMAT_AA_RR(0:7, NCHANNELS, NCH_MAX, NCH_MAX))
        ALLOCATE(FMAT_AA_IR(0:7, NCHANNELS, NCH_MAX, NCH_MAX))
        ALLOCATE(FMAT_AA_RI(0:7, NCHANNELS, NCH_MAX, NCH_MAX))
        ALLOCATE(FMAT_AA_II(0:7, NCHANNELS, NCH_MAX, NCH_MAX))
      ENDIF
    ENDIF
      ...
      !      IRREGOLARE-REGOLARE (APE1), REGOLARE-REGOLARE (APE2), IRREGOLARE-IRREGOLARE (APE3)
          ...
          ELSE
            ...
            DO IPOT = 0, 7
              ...
              FMAT_AA_RR(IPOT,ICH,IAB,IAK) = B5_SINGLE(NX,H5,FUN2,1)
              FMAT_AA_RI(IPOT,ICH,IAB,IAK) = B5_SINGLE(NX,H5,FUN,1)
              FMAT_AA_IR(IPOT,ICH,IAB,IAK) = B5_SINGLE(NX,H5,FUN1,1)
              FMAT_AA_II(IPOT,ICH,IAB,IAK) = B5_SINGLE(NX,H5,FUN3,1)
            ENDDO
          ENDIF
