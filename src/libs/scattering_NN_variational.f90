...
! S-MATRIX
  ...
  IF (FIRST_CALL) THEN
    ...
    NNNEW = VAR_P%NNL * NCH
    ALLOCATE(C(NNNEW, NNNEW), CC(NNNEW, NNNEW), CCC(NNNEW, NNNEW))
    ALLOCATE(CAR(NNNEW,NCH), CAI(NNNEW,NCH))
    ALLOCATE(CARR(NNNEW), CAII(NNNEW))
    ALLOCATE(XRCOEFF(NCH,NNNEW), XICOEFF(NCH,NNNEW))
    ALLOCATE(IPIV(NNNEW))
...
! Preparing the matrix elements for the diagonalization
  ...
  ! Evaluating for the "c_{n, alpha}" coefficients
    CALL DGESV(NDIM, 1, CC , NNNEW, IPIV, CARR, NNNEW, INFO)
    CALL HANDLE_INFO_ERROR()  ! Handle the error after the first DGESV call
    IF (PRINT_I) WRITE(*,*) "INFO: ", INFO
    CALL DGESV(NDIM, 1, CCC, NNNEW, IPIV, CAII, NNNEW, INFO)
...
! Calculating the phase shifts and mixing angles in the Stapp convention
  ...
  CONTAINS
    ...
    SUBROUTINE R_SECOND_ORDER()
      ...
      DOUBLE PRECISION, ALLOCATABLE, SAVE :: ASS(:,:), ANNS(:,:)

      IF (FIRST_CALL) THEN
        ALLOCATE(XRCOEFV(NNNEW, NCH_MAX), XICOEFV(NNNEW, NCH))
        ALLOCATE(BD5(NCH, NNNEW), BD6(NCH, NNNEW))
        ALLOCATE( CD0(NCH, NCH), CD1(NCH, NCH), CD2(NCH, NCH), CD3(NCH, NCH), CD4(NCH, NCH), &
                  CD5(NCH, NCH), CD6(NCH, NCH), CD7(NCH, NCH), CD8(NCH, NCH))
        ALLOCATE(ASS(NCH, NCH), ANNS(NCH, NCH))
        ALLOCATE(RD0(NCH, NCH), RD2(NCH, NCH), RD3(NCH, NCH))
        ALLOCATE(RCI(NCH, NNNEW), RCIV(NNNEW, NCH))
        FIRST_CALL = .FALSE.
      ENDIF
  ...
  SUBROUTINE CORE_CORE_MATRIX_ELEMENTS(AM)
    ...
    IF (FIRST_CALL) THEN
      ...
      ALLOCATE(AKEM(NNNEW,NNNEW), APEM(NNNEW,NNNEW))
      ALLOCATE(HM(NNNEW,NNNEW), AXX(NNNEW,NNNEW))
  ...
  ! Initialize grid with r values
    ...
    IF (FIRST_CALL) THEN
      ALLOCATE(AXXM1(NNNEW, NCH), AKEM1(NNNEW, NCH), APEM(NNNEW,NCH), APEM1(NNNEW, NCH))
