MODULE SCATTERING_NN_VARIATIONAL
  ...
  DOUBLE PRECISION, ALLOCATABLE :: HCC(:,:,:), NCC(:,:,:)
...
! S-MATRIX
  ...
  ENDIF
  CALL REALLOCATE_3D_1(NCC, NE, NNN, NNN)
  CALL REALLOCATE_3D_1(HCC, NCHANNELS, NNN, NNN)
...
! Calculating the phase shifts and mixing angles in the Stapp convention
  ...
  SUBROUTINE CORE_CORE_MATRIX_ELEMENTS(AM, RESET, CLEAN)
    ...
    IF (PRESENT(CLEAN)) THEN
      IF (CLEAN) THEN
        IF(ALLOCATED(AKEM)) DEALLOCATE(AKEM, APEM, FUN)
        RETURN
      ENDIF 
    ENDIF
    ...
    NX = VAR_P%NX_CC
    IF (RESET) THEN
      CALL REALLOCATE_2D_1(AKEM, NNN, NNN)
      CALL REALLOCATE_3D_1(APEM, NCHANNELS, NNN, NNN)
    ...
    ! SI CALCOLA ENERGIA POTENZIALE
          DO ICH =1, NCHANNELS
            SUM = ZERO
            FUN = V0_CC(IL,:)*V0_CC(IR,:)*V_CC(ICH,:,IAB,IAK)
            DO I=1,NX
              SUM = SUM + XX_CC(I)*XX_CC(I)*FUN(I)*A_CC(I)
            ENDDO
            APEM(ICH,IB,IK) = SUM/GAMMA
          ENDDO
      ...
      ENDDO ! IAB
      DO ICH=1, NCHANNELS
        HCC(ICH,:,:) = ( AKEM + APEM(ICH,:,:) )
      ENDDO
  
      IF (PRINT_I) WRITE(*,*)'POTENTIAL', APEM(CH_INDEX,1,1)
      DEALLOCATE(FUN)
      DEALLOCATE(AKEM, APEM)
    ENDIF

    IE = FIND_ENERGY_INDEX(VAR_P%E)
    AM = (HCC(CH_INDEX,:,:) - NCC(IE,:,:))/ HTM
