...
CONTAINS
  SUBROUTINE SET_ENERGIES(ENERGIES)
    ...
    CALL REALLOCATE_1D_1(ENERGIES_, SIZE(ENERGIES))
    ENERGIES_ = ENERGIES
    ENERGIES_SET = .TRUE.
  ...
  SUBROUTINE SET_CHANNELS(CHANNELS)
    ...
    ALLOCATE(CHANNELS_(SIZE(CHANNELS)))
    CHANNELS_ = CHANNELS
...
! Calculating the phase shifts and mixing angles in the Stapp convention
  ...
  SUBROUTINE CORE_CORE_MATRIX_ELEMENTS(AM, RESET)
    ...
    ! SI CALCOLA ENERGIA POTENZIALE
          SUM = ZERO
          FUN = V0_CC(IL,:)*V0_CC(IR,:)*V_CC(CH_INDEX,:,IAB,IAK)
          DO I=1,NX
  ...
  SUBROUTINE PREPARE_POTENTIAL(CHANNELS)
    IMPLICIT NONE
    TYPE(SCATTERING_CHANNEL), INTENT(IN) :: CHANNELS(:)
    INTEGER :: NC, NEQ_C, ICH, IR, IEQ, L, S, J, T, TZ, T1Z, T2Z, IL, NL
    LOGICAL :: COUPLED
