...
! S-MATRIX
  ...
  ENDIF
  CALL REALLOCATE_3D_1(ENCC, NE, NNN, NNN)
...
! Calculating the phase shifts and mixing angles in the Stapp convention
  ...
  SUBROUTINE CORE_CORE_MATRIX_ELEMENTS(AM, RESET, CLEAN)
    ...
    INTEGER :: I, NX, IE, ICH
    INTEGER :: NNL
    INTEGER :: COMMON_INDEX(NCH_MAX, NNE), LIK, IAB, IAK, IL, IR, IB, IK
    ...
    IF (PRESENT(CLEAN)) THEN
      IF (CLEAN) THEN
        IF(ALLOCATED(KIN_MATRIX)) DEALLOCATE(KIN_MATRIX, POT_MATRIX, INTEGRAND)
        RETURN
      ENDIF 
    ENDIF
    ...
    IF (RESET) THEN
      ...
      CALL REALLOCATE_2D_1(KIN_MATRIX, NNN, NNN)
      CALL REALLOCATE_3D_1(POT_MATRIX, NCHANNELS, NNN, NNN)
    ...
    ! SI CALCOLA ENERGIA POTENZIALE
          DO ICH =1, NCHANNELS
            ...
            DO I=1,NX
              SUM = SUM + XX_CC(I)*XX_CC(I)*INTEGRAND(I)*A_CC(I)
            ENDDO
      ...
      ENDDO ! IAB
      DO ICH=1, NCHANNELS
        HCC(ICH,:,:) = ( KIN_MATRIX + POT_MATRIX(ICH,:,:) )
      ENDDO
